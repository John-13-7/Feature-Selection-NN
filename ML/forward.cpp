...
double return_distance_forward(set<int> &features, vector<vector<double>> &data, int first_row, int second_row);
int nearest_neighbor_forward(set<int> &features, vector<vector<double>> &data, int excluded_row);
...
void forward()
{
    ...
    for (int j = 1; j < data[0].size(); j++) // Start from 1 as first column is label
    {
        ...
        for (int k = 1; k < data[0].size(); k++)
        {
            ...
            {
                int nearest = nearest_neighbor_forward(copy, data, z);
...
int nearest_neighbor_forward(set<int> &features, vector<vector<double>> &data, int excluded_row)
{
    ...
    for (int i = 0; i < data.size(); ++i)
    {
        if (i != excluded_row)
        {
            double distance = return_distance_forward(features, data, i, excluded_row);
